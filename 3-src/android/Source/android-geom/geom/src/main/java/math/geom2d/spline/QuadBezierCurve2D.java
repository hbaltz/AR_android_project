 ...
 * @author Legland
 */
public class QuadBezierCurve2D extends AbstractSmoothCurve2D implements
    ...
    public QuadBezierCurve2D(double[][] coefs) {
        this(coefs[0][0], coefs[1][0], coefs[0][0] + coefs[0][1] / 2.0,
                coefs[1][0] + coefs[1][1] / 2.0, coefs[0][0] + coefs[0][1]
                        + coefs[0][2], coefs[1][0] + coefs[1][1] + coefs[1][2]);
    }
    ...
    public QuadBezierCurve2D(Point2D[] pts) {
        this(pts[0].x(), pts[0].y(), pts[1].x(), pts[1].y(), pts[2].x(), pts[2]
                .y());
    }
    ...
    public QuadBezierCurve2D reverse() {
        return new QuadBezierCurve2D(this.lastPoint(), this.getControl(),
                this.firstPoint());
    }
    ...
    public CurveSet2D<? extends QuadBezierCurve2D> clip(Box2D box) {
        ...
        CurveArray2D<QuadBezierCurve2D> result = new CurveArray2D<QuadBezierCurve2D>(
                set.size());
    ...
    public QuadBezierCurve2D transform(AffineTransform2D trans) {
        return new QuadBezierCurve2D(trans.transform(this.firstPoint()),
                trans.transform(this.getControl()), trans.transform(this
                        .lastPoint()));
    }
    ...
    public Path appendPath(Path path) {
        Point2D p2 = this.getControl();
        Point2D p3 = this.lastPoint();
        path.quadTo((float) p2.x(), (float) p2.y(), (float) p3.x(),
                (float) p3.y());
        return path;
    }
