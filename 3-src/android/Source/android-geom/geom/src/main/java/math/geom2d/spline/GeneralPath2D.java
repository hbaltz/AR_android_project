...
public class GeneralPath2D implements Curve2D {
    ...
    private Collection<SmoothCurve2D> smoothCurves() {
        ...
        for (Segment seg : this.segments) {
            ...
            case CUBIC:
                pts = seg.controlPoints();
                curves.add(new CubicBezierCurve2D(lastControl, pts[0], pts[1],
                        pts[2]));
                lastControl = pts[2];
                break;
            ...
            default:
                throw new RuntimeException("Unknown Path segment type: "
                        + seg.type());
            }
        }
    ...
    private SmoothCurve2D segmentCurve(int index) {
        ...
        if (index > n - 2)
            throw new IllegalArgumentException(
                    "Index must be lower than segment number");
        ...
        for (int i = 0; i < index + 2; i++) {
            ...
            case CUBIC:
                pts = seg.controlPoints();
                curve = new CubicBezierCurve2D(lastControl, pts[0], pts[1],
                        pts[2]);
                lastControl = pts[2];
                break;
    ...
    public Point2D point(double t) {
        ...
        if (index == this.segments.size() - 1
                && Math.abs(t - index) < Shape2D.ACCURACY)
        ...
        if (curve == null) {
            throw new RuntimeException(
                    "Can not manage position for MOVE Path segments");
        }
    ...
    private ArrayList<ContinuousCurve2D> splitContinuousCurves() {
        ...
        int n = this.segments.size();
        ArrayList<ContinuousCurve2D> curveList = new ArrayList<ContinuousCurve2D>(
                n);
        ...
        for (Segment seg : this.segments) {
            ...
            case CUBIC:
                pts = seg.controlPoints();
                curve.add(new CubicBezierCurve2D(lastControl, pts[0], pts[1],
                        pts[2]));
                lastControl = pts[2];
                break;
    ...
    public boolean almostEquals(GeometricObject2D obj, double eps) {
        ...
        for (int i = 0; i < this.segments.size(); i++) {
            ...
            if (pts1.length != pts2.length)
                throw new RuntimeException(
                        "Two path segments have type but different number of control points");
    ...
    private class CubicSegment implements Segment {
        ...
        public void updatePath(Path path) {
            path.cubicTo((float) p1.x(), (float) p1.y(), (float) p2.x(),
