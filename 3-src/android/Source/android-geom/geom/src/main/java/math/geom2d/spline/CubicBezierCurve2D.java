 ...
 * @author Legland
 */
public class CubicBezierCurve2D extends AbstractSmoothCurve2D implements
    ...
    public CubicBezierCurve2D(double[][] coefs) {
        this(coefs[0][0], coefs[1][0], coefs[0][0] + coefs[0][1] / 3.0,
                coefs[1][0] + coefs[1][1] / 3.0, coefs[0][0] + 2 * coefs[0][1]
                        / 3.0 + coefs[0][2] / 3.0, coefs[1][0] + 2
    ...
    public CubicBezierCurve2D(Point2D p1, Point2D ctrl1, Point2D ctrl2,
            Point2D p2) {
        this(p1.x(), p1.y(), ctrl1.x(), ctrl1.y(), ctrl2.x(), ctrl2.y(),
                p2.x(), p2.y());
    }
    ...
    public CubicBezierCurve2D(Point2D p1, Vector2D v1, Point2D p2, Vector2D v2) {
        this(p1.x(), p1.y(), p1.x() + v1.x() / 3, p1.y() + v1.y() / 3, p2.x()
                - v2.x() / 3, p2.y() - v2.y() / 3, p2.x(), p2.y());
    }
     ...
     */
    public CubicBezierCurve2D(double x1, double y1, double xctrl1,
            double yctrl1, double xctrl2, double yctrl2, double x2, double y2) {
        this.x1 = x1;
        this.y1 = y1;
        this.ctrlx1 = xctrl1;
        this.ctrly1 = yctrl1;
    ...
    public CubicBezierCurve2D reverse() {
        return new CubicBezierCurve2D(this.lastPoint(), this.getControl1(),
    ...
    public CurveSet2D<? extends CubicBezierCurve2D> clip(Box2D box) {
        ...
        CurveArray2D<CubicBezierCurve2D> result = new CurveArray2D<CubicBezierCurve2D>(
                set.size());
    ...
    public CubicBezierCurve2D transform(AffineTransform2D trans) {
        return new CubicBezierCurve2D(trans.transform(this.firstPoint()),
                trans.transform(this.getControl1()), trans.transform(this
                        .getControl2()), trans.transform(this.lastPoint()));
    }

    public Path appendPath(Path path) {
        path.moveTo((float) x1, (float) y1);
        path.cubicTo((float) ctrlx1, (float) ctrly1, (float) ctrlx2,
    ...
    public CubicBezierCurve2D clone() {
        return new CubicBezierCurve2D(x1, y1, ctrlx1, ctrly1, ctrlx2, ctrly2,
                x2, y2);
    }
}
