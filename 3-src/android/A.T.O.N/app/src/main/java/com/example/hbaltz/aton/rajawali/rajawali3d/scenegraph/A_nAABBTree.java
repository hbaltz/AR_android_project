 ...
 */
package com.example.hbaltz.aton.rajawali.rajawali3d.scenegraph;
...
import org.rajawali3d.scenegraph.*;
 ...
 */
public abstract class A_nAABBTree extends BoundingBox implements org.rajawali3d.scenegraph.IGraphNode {
	...
	protected void grow() {
		...
		if (mSplit) {
			for (int i = 0; i < CHILD_COUNT; ++i) {
				((org.rajawali3d.scenegraph.Octree) mChildren[i]).setChildRegion(i, mChildLengths);
			}
		}
		for (int i = 0; i < members_count; ++i) {
	...
	public synchronized void removeObject(IGraphNodeMember object) {
		...
		org.rajawali3d.scenegraph.IGraphNode container = object.getGraphNode();
		if (container == null) {
